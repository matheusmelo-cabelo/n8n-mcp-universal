...
/**
 * Validate URL format with enhanced checks including SSRF protection
 */
function isValidUrl(url: string): boolean {
  // Use SSRF protection for robust validation
  // Default to 'permissive' to allow local/intranet n8n instances (backward compatibility)
  // but still block Cloud Metadata endpoints.
  const mode = (process.env.N8N_API_SECURITY_MODE || 'permissive') as SecurityMode;
  const validation = SSRFProtection.validateUrlSync(url, mode);
